...
turning.define('modal:create-task').test(async () => {
  await expect(page).toMatchElement('.create-task-modal');
});

turning.define('task:create:procedure-selected').test(async () => {
...
turning
  ...
  .by('clicking "+" on workbench', async () => {
...
turning
  ...
  .to(['task:create:procedure-selected'])
  .by('select the second procedure', async () => {
