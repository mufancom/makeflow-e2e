...
turning.define('/website/home').test(async context => {
  let page = await context.getPage();

  await page.waitFor('.home-view');
});

turning.define('/website/login').test(async context => {
  let page = await context.getPage();

  await page.waitFor('.login-view');
});

turning.define('/website/sign-up/create-account').test(async context => {
  let page = await context.getPage();
...
turning.define('/website/sign-up/create-organization').test(async context => {
  let page = await context.getPage();
...
turning.define('/website/sign-up/complete-user-profile').test(async context => {
  let page = await context.getPage();
